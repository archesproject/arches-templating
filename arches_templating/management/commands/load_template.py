...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with open(
                os.path.join(template_directory, config["file"]), "rb"
            ) as template_file:
                saved_template_file = default_storage.save(
                    template_file_name, File(template_file)
                )
        ...
        try:
            ...
            KeyError
        ):  # optional, but if not provided we will always create a new template
            uuid_obj = uuid.uuid4()
            template_id = str(uuid_obj)
        except ValueError:
        ...
        try:
            preview_file = glob.glob(
                os.path.join(template_directory, config["preview"])
            )[0]
            with open(preview_file, "rb") as source_file:
                saved_preview_file = default_storage.save(
                    os.path.basename(preview_file), File(source_file)
                )
        except (KeyError, FileNotFoundError):
            pass  # preview file need not exist

        try:
            thumbnail_file = glob.glob(
                os.path.join(template_directory, config["thumbnail"])
            )[0]

            with open(thumbnail_file, "rb") as source_file:
                saved_thumbnail_file = default_storage.save(
                    os.path.basename(thumbnail_file), File(source_file)
                )
        except (KeyError, FileNotFoundError):
