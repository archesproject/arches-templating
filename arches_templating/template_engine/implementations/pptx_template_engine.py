...
class PptxTemplateEngine(TemplateEngine):
    ...
    def match_text(self, container, parent, cell=None):
        ...
        for match in re.findall(self.regex, container.text):
            parsed_tags.append(
                (match, {"container": container, "parent": parent, "cell": None})
            )
        return parsed_tags
                        ...
                        try:
                            ...
                            while current_row < row_length:
                                ...
                                while current_column < column_length:
                                    current_cell = table.cell(
                                        current_row, current_column
                                    )
                                    parsed_tags += self.iterate_over_container(
                                        current_cell, table
                                    )
                                    current_column += 1
                                current_row += 1
        ...
        for tag in tags:
            ...
            if tag.type == TemplateTagType.CONTEXT:
                ...
                if tag.has_rows:
                    ...
                    parent_table = tag.context_children_template[-1].optional_keys[
                        "parent"
                    ]
                    ...
                    lead_matches_len = len(
                        self.regex.findall(parent_table.cell(0, 0).text)
                    )
                    ...
                    if lead_matches_len > 1:
                        ...
                        primary_cell.text = primary_cell.text.replace(
                            tag.raw, primary_cell.text
                        )
                    else:
                ...
                if tag.render:
                    ...
                    found_if_end = False
                    for item in self.iterate_over_shapes_in_slide(
                        block._parent._parent
                    ):
                        if item._element == block._element:
                            found_if_start = True
                        if (
                            item._element
                            == tag.end_tag.optional_keys["container"]._element
                        ):
                            found_if_end = True
                            try:
                                for child_tag in PptxTemplateEngine.get_all_child_tags(
                                    tag
                                ):
