...
class XlsxTemplateEngine(TemplateEngine):
    ...
    def increment_column(column: str):
        return XlsxTemplateEngine.column_from_offset(
            XlsxTemplateEngine.offset_from_column(column) + 1
        )
        ...
        if match:
            ...
            while current_row <= end_row:
                ...
                while current_col <= end_col_offset:
                    current_cell = XlsxTemplateEngine.column_from_offset(
                        current_col
                    ) + str(current_row)
                            ...
                            parsed_tags.append(
                                (
                                    match,
                                    {
                                        ...
                                        "sheet": sheet,
                                        "row": current_row,
                                        "column": XlsxTemplateEngine.column_from_offset(
                                            current_col
                                        ),
                                    },
                                )
        ...
        for tag in tags:
            ...
            if tag.type == TemplateTagType.CONTEXT:
                if tag.has_rows:
                    ...
                    current_row = (
                        tag.context_children_template[-1].optional_keys["row"]
                        + rowshift
                    )
                    ...
                    for child in tag.children:
                        if child.type == TemplateTagType.ROWEND:
                            ...
                            sheet[
                                child.optional_keys["column"] + str(current_row)
                            ].value = child.value
                else:
                ...
                if tag.render:
                    ...
                    tag.end_tag.optional_keys["cell"].value = tag.end_tag.optional_keys[
                        "cell"
                    ].value.replace(tag.end_tag.raw, "")
                    rowshift = self.replace_tags(tag.children, rowshift)
                else:
                    # delete rows between tags
                    if row != tag.end_tag.optional_keys["row"] + rowshift:
                        sheet.delete_rows(
                            row, tag.end_tag.optional_keys["row"] + rowshift
                        )
                        rowshift += row - (tag.end_tag.optional_keys["row"] + rowshift)

                    elif (
                        row == tag.end_tag.optional_keys["row"] + rowshift
                        and column != tag.end_tag.optional_keys["column"]
                    ):
                        current_col = column
                        end_col = tag.end_tag.optional_keys["column"]
                        while current_col != end_col:
                            sheet[current_col + str(row)].value = ""
                            current_col = XlsxTemplateEngine.increment_column(
                                current_col
                            )
                    else:
                        cell.value = ""
