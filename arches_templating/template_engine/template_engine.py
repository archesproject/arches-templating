...
class TemplateEngine(object):
    ...
    def extract_tags(self, template) -> List[TemplateTag]:
        ...
        for match in tag_matches:
            ...
            if tag_type == TemplateTagType.CONTEXT or tag_type == TemplateTagType.IF:
                ...
                context[-1].end_tag = TemplateTag(
                    raw, tag_type, attributes, optional_keys
                )
                context.pop()
            elif len(context) == 0:
                ...
                context[-1].children.append(
                    TemplateTag(raw, tag_type, attributes, optional_keys)
                )
        return tags
        ...
        for tag in tags:
            if tag.type == TemplateTagType.VALUE:
                ...
                if path_value is not None:
                    if path_value.startswith("/"):
                        ...
                        tag.value = self.traverse_dictionary(
                            path_value, current_root_context
                        )
                    else:
            ...
            if tag.type == TemplateTagType.IMAGE:
                ...
                if path_value is not None:
                    if path_value.startswith("/"):
                        # any path that starts with "/" will be tied to the root context.  We can be more explicit in tag attributes later if need be
                        image_value = self.traverse_dictionary(
                            path_value, current_root_context
                        )
                    else:
                        ...
                        if re.match("^http", image_value):
                            if re.search(r"\/temp_file\/", image_value):
                                ...
                                if (
                                    ...
                                    internal_base_url = urlparse(
                                        root_context["internal_base_url"]
                                    )
                                    parsed_url = parsed_url._replace(
                                        netloc=internal_base_url.netloc
                                    )
                                image_value = urlunparse(parsed_url)

                            tag.value = "data:image/jpeg;base64," + base64.b64encode(
                                requests.get(image_value).content
                            ).decode("utf-8")
                        else:
                            tag.value = image_value
                    except (
                        TypeError
                    ) as e:  # when image_value is null, the render failed
                        raise Exception(
                            "Could not get a URL for a requested image, image_value was not a string",
                            e,
                        ) from e
                ...
                if path_value is not None:
                    if path_value.startswith("/"):
                        # any path that starts with "/" will be tied to the root context.  We can be more explicit in tag attributes later if need be
                        new_context = self.traverse_dictionary(
                            path_value, current_root_context
                        )
                    else:
                    ...
                    if isinstance(new_context, List) and index_value:
                        ...
                        for item in new_context:
                            tag.children.extend(
                                self.get_tag_values(
                                    copy.deepcopy(tag.context_children_template),
                                    item,
                                    current_root_context,
                                )
                            )
    ...
    def document_replace(self, template, context):
        ...
        result_file_stream, mime_type, incomplete = self.create_file(
            expanded_tags, template
        )
        ...
        while incomplete:
            result_file_stream, mime_type, incomplete = self.document_replace(
                result_file_stream, context
            )
